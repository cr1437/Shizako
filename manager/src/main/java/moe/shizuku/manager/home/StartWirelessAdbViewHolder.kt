...
class StartWirelessAdbViewHolder(binding: HomeStartWirelessAdbBinding, root: View) :
        ...
        if (port > 0) {
            ...
            context.startActivity(
                MainActivity.destinationIntent(
                    ...
                    Bundle().apply {
                        putBoolean(StarterFragment.EXTRA_IS_ROOT, false)
                        putString(StarterFragment.EXTRA_HOST, host)
                        putInt(StarterFragment.EXTRA_PORT, port)
                    }
                )
            )
        } else {
    ...
    private fun onPairClicked(context: Context) {
        if ((context.display?.displayId ?: -1) > 0) {
            ...
            context.startActivity(MainActivity.destinationIntent(context, R.id.adb_pairing_fragment))
        }
    }
}
