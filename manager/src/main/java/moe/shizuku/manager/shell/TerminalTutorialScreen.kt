...
sealed interface DeployState {
    ...
    data class Success(val targetDir: String, val inTermuxPath: Boolean) : DeployState
...
fun TerminalTutorialScreen(
    ...
    HintPage(
        ...
        item { Box(modifier = Modifier.animateItem().itemEntrance(0)) { DeployCard(state, palette, onDeploy, onOpenDocs) } }
        item { Box(modifier = Modifier.animateItem().itemEntrance(1)) { ManualCard(palette, onExportFiles) } }
        item { Box(modifier = Modifier.animateItem().itemEntrance(2)) { NotesCard(state, palette) } }
    }
}
            ...
            AnimatedContent(
                ...
                when (deploy) {
                    ...
                    is DeployState.Running -> DeployStatusRow(
                        palette = palette,
                        // AnimatedVectorDrawable：转圈（自己会转，不用 Compose 驱动）
                        iconRes = R.drawable.ic_deploy_spinner,
